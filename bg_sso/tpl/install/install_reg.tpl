...
{include "include/install_head.tpl" cfg=$cfg}
	...
	<div class="page_head">
		...
		&raquo;
		{$lang.page.installReg}
	</div>

	<div class="page_body">
		<form name="instal_form_reg" id="instal_form_reg">
...
{include "include/install_foot.tpl" cfg=$cfg}
	...
	var opts_submit_form = { ajax_url: "{$smarty.const.BG_URL_INSATLL}ajax.php?mod=install", btn_text: "{$lang.btn.installNext}", btn_url: "{$smarty.const.BG_URL_INSATLL}install.php?mod=install&act_get=admin" };

	$(document).ready(function(){
		var obj_validator_form = $("#instal_form_reg").baigoValidator(opts_validator_form);
		var obj_submit_form = $("#instal_form_reg").baigoSubmit(opts_submit_form);
		$("#go_pre").click(function(){
			window.location.href = "{$smarty.const.BG_URL_INSATLL}install.php?mod=install&act_get=base";
		});
