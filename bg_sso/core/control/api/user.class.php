...
class API_USER {
    ...
    function api_login() {
        ...
        //$_arr_userRow["user_is_upgrade"] 字段是判断密码加密方式是否已经升级, 系统在升级后, 第一次登录时进行自动判断, 然后升级加密方式
        if (fn_baigoEncrypt($_arr_userSubmit["user_pass"], $_arr_userRow["user_rand"], true, $_arr_userRow["user_is_upgrade"]) != $_arr_userRow["user_pass"]) {
            $_arr_return = array(
                "alert" => "x010213",
            );
            $this->obj_api->halt_re($_arr_return);
