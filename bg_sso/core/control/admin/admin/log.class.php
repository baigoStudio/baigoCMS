...
class CONTROL_LOG {
	...
	*/
	function ctl_show() {
		...
		$_arr_logTargets = json_decode($_arr_logRow["log_targets"], true);

		foreach ($_arr_logTargets as $_key=>$_value) {
			switch ($_arr_logRow["log_target_type"]) {
				case "admin":
					$_arr_adminRow = $this->mdl_admin->mdl_read($_value["admin_id"]);
					$_arr_logTargets[$_key]["target_id"]   = $_value["admin_id"];
					$_arr_logTargets[$_key]["target_name"] = $_arr_adminRow["admin_name"];
				break;

				case "user":
					$_arr_userRow = $this->mdl_user->mdl_read($_value["user_id"]);
					$_arr_logTargets[$_key]["target_id"]   = $_value["user_id"];
					$_arr_logTargets[$_key]["target_name"] = $_arr_userRow["user_name"];
				break;

				case "app":
					$_arr_appRow = $this->mdl_app->mdl_read($_value["app_id"]);
					$_arr_logTargets[$_key]["target_id"]   = $_value["app_id"];
					$_arr_logTargets[$_key]["target_name"] = $_arr_appRow["app_name"];
				break;

				case "log":
					$_arr_logTargets[$_key]["target_id"]   = $_value["log_id"];
					$_arr_logTargets[$_key]["target_name"] = "log";
				break;

				case "opt":
					$_arr_logTargets[$_key]["target_id"] = $_value["opt_key"];
					$_arr_logTargets[$_key]["target_name"] = $_value["opt_key"];
				break;
			}
		}

		$_arr_logRow["log_targets"] = $_arr_logTargets;
