...
include_once(BG_PATH_CONTROL_ADMIN . "admin/logon.class.php"); //载入登录控制器
...
switch ($act_post) {
	case "login":
		...
		if ($arr_logonRow["str_alert"] != "y020201") {
			header("Location: " . BG_URL_ADMIN . "admin.php?mod=logon&act_get=logon&forward=" . $arr_logonRow["forward"] . "&alert=" . $arr_logonRow["str_alert"]);
		} else {
