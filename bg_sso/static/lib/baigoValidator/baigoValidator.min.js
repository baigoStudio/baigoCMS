...
var _err=0;var thisForm=$(this);var el=this;var _status;var _numLenth=0;var _iii;var _chkCode;var _reg;var defaults={class_name:{success:"baigoValidator-success",err:"baigoValidator-err",loading:"baigoValidator-loading"},class_attach:{success:"has-success",err:"has-error"}};var opts=$.extend(true,defaults,options);var outPut=function(_msg_selector,_group_selector,_msg,_class,_attach_class){$(_msg_selector).empty();$(_msg_selector).append(_msg);$(_msg_selector).attr("class",_class);$(_msg_selector).show();if(typeof _group_selector!="undefined"&&_group_selector.length>0){$(_group_selector).removeClass(opts.class_attach.success+" "+opts.class_attach.err);$(_group_selector).addClass(_attach_class);}};var getLength=function(_str){if(_str){_numLenth=0;for(_iii=0;_iii<_str.length;_iii++){_chkCode=_str.charCodeAt(_iii);if(_chkCode<0x007f){_numLenth++;}else if(_chkCode<=0x07ff){_numLenth+=2;}else if(_chkCode<=0xffff){_numLenth+=3;}else{_numLenth+=4;}}}
...
break;case"ajax":var _ajax=_set_data.ajax;_str=$(_validate_selector).val();_status=isAjax(_str,_len,_validate,_ajax,_msg,_validate_selector);break;case"confirm":var _str_target=$(_validate.target).val();_str=$(_validate_selector).val();_status=isConfirm(_str,_str_target,_len);break;default:_str=$(_validate_selector).val();_status=isText(_str,_len,_validate.format);break;}
switch(_status){case"ok":outPut(_msg.selector,_validate.group,"&nbsp;",opts.class_name.success,opts.class_attach.success);_result=true;break;default:$(_validate_selector).focus();outPut(_msg.selector,_validate.group,_msg[_status],opts.class_name.err,opts.class_attach.err);_result=false;break;}
