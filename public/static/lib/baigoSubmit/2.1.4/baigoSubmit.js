  ...
  $.fn.baigoSubmit = function(options) {
    ...
    var process = {
      ...
      output: function(status, rcode, msg, attach_value, extra) { //调用弹出框
        ...
        switch(status) {
          case 'pre':
            this.modalOutput('submitting', 'submitting', opts.msg_text.submitting);
          ...
          case 'err':
            this.modalOutput('err', 'err', opts.msg_text.err + '! ' + msg);
          break;
          ...
          case 'success':
            ...
            if (_rcode_status == 'y') {
              _status     = 'success';
              _icon       = 'success';
              _jump_url   = this.jumpUrl(attach_value);
            ...
            this.modalOutput(_status, _icon, _msg_html, extra);
      ...
      submitFunc: function(callback) {
        ...
        $.ajax({
          ...
          beforeSend: function(){
            this.output('pre'); //输出正在提交
          }, //输出消息
          error: function(result){
            this.output('err', '', result.status + ' ' + result.statusText); //输出消息
            setTimeout(function(){
              this.output();
            }, opts.modal.delay);
          },
          success: function(result){ //读取返回结果
            ...
            this.output('success', result.rcode, result.msg, _attach_value, result.extra); //输出消息
