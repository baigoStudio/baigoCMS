...
class Mysql extends Connector {
    ...
    function where($where, $exp = '', $value = '', $param = '', $type = '') {
        $_arr_result = $this->obj_builder->where($where, $exp, $value, $param, $type);
    ...
    function select($field = '', $all = true) {
        ...
        if ($this->_fetchSql === true || $this->configDebug === 'trace') {
            $_str_sqlLog = $this->fetchBind($_str_sql, $this->_bind);
        }
    ...
    function insert($field, $value = '', $param = '', $type = '') {
        ...
        if ($this->_fetchSql === true || $this->configDebug === 'trace') {
            $_str_sqlLog = $this->fetchBind($_arr_result['sql'], $_arr_result['bind']);
        }
    ...
    function update($field, $value = '', $param = '', $type = '') {
        ...
        if ($this->_fetchSql === true || $this->configDebug === 'trace') {
            $_str_sqlLog = $this->fetchBind($_arr_result['sql'], $this->_bind);
            $_str_sqlLog = $this->fetchBind($_arr_result['sql'], $_arr_result['bind']);
        }
    ...
    function buildSql() {
        ...
        $_str_sql = trim($this->fetchBind($_str_sql, $this->_bind));
