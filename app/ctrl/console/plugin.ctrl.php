...
class Plugin extends Ctrl {
    ...
    function index() {
        ...
        if (!isset($this->groupAllow['plugin']['plugin']['browse']) && !$this->isSuper) { //判断权限
        ...
        foreach ($_arr_pluginRows as $_key=>$_value) {
            ...
            if ($_mix_pluginOpts) {
                $_str_pluginOpts  = $this->obj_file->fileRead($_mix_pluginOpts);
    ...
    function form() {
        ...
        if (isset($this->configPlugin[$_str_pluginDir])) {
            if (!isset($this->groupAllow['plugin']['plugin']['edit']) && !$this->isSuper) { //判断权限
            ...
            if (!isset($this->groupAllow['plugin']['plugin']['install']) && !$this->isSuper) { //判断权限
    ...
    function opts() {
        ...
        if (!isset($this->groupAllow['plugin']['plugin']['option']) && !$this->isSuper) { //判断权限
            return $this->error('You do not have permission', 'x190301');
        }
    ...
    function optsSubmit() {
        ...
        if (!isset($this->groupAllow['plugin']['plugin']['option']) && !$this->isSuper) {
    ...
    function uninstall() {
        ...
        if (!isset($this->groupAllow['plugin']['plugin']['uninstall']) && !$this->isSuper) { //判断权限
