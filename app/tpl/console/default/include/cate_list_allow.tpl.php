    <?php function cate_list_allow($arr_cateRows, $cate_allow, $lang = '', $admin_allow_cate = array(), $group_allow_article = array(), $admin_type = '', $is_edit = true) {
        if (!empty($arr_cateRows)) {
            foreach ($arr_cateRows as $_key=>$_value) { ?>
                    ...
                    <td class="bg-child-<?php echo $_value['cate_level']; ?>">
                        ...
                        <div>
                            ...
                            foreach ($cate_allow as $_key_s=>$_value_s) {
                                ...
                                <?php } else { ?>
                                    <span>
                                        <span class="fas fa-<?php if (isset($admin_allow_cate[$_value['cate_id']][$_key_s]) || isset($group_allow_article[$_key_s]) || $admin_type == 'super') { ?>check-circle text-success<?php } else { ?>times-circle text-danger<?php } ?>"></span>
                                        <?php echo $lang->get($_value_s, 'console.common'); ?>
                                    </span>
                                <?php }
                            } ?>
                ...
                <?php if (isset($_value['cate_childs'])) {
                    cate_list_allow($_value['cate_childs'], $cate_allow, $lang, $admin_allow_cate, $group_allow_article, $admin_type, $is_edit);
                }
            }
        }
    }
    ...
    cate_list_allow($_arr_cateRows, $cate_allow, $lang, $_arr_adminRow['admin_allow_cate'], $_arr_groupAllowArticle, $_arr_adminRow['admin_type'], $_is_edit);