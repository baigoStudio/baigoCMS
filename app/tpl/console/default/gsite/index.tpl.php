...
include($cfg['pathInclude'] . 'console_head' . GK_EXT_TPL); ?>
    ...
    <div class="d-flex justify-content-between">
        <nav class="nav mb-3">
            <a href="<?php echo $route_console; ?>gsite/form/" class="nav-link">
                <span class="fas fa-plus"></span>
                <?php echo $lang->get('Add'); ?>
            </a>
        </nav>
        <form name="gsite_search" id="gsite_search" class="d-none d-lg-block" action="<?php echo $route_console; ?>gsite/index/">
    ...
    <?php if (!empty($search['key']) || !empty($search['status'])) { ?>
        <div class="mb-3 text-right">
            ...
            <a href="<?php echo $route_console; ?>gsite/index/" class="badge badge-danger badge-pill">
    ...
    <form name="gsite_list" id="gsite_list" action="<?php echo $route_console; ?>gsite/status/">
                    ...
                    <?php foreach ($gsiteRows as $key=>$value) { ?>
                        <tr class="bg-manage-tr">
                            <td class="text-nowrap bg-td-xs">
                                <div class="form-check">
                                    <input type="checkbox" name="gsite_ids[]" value="<?php echo $value['gsite_id']; ?>" id="gsite_id_<?php echo $value['gsite_id']; ?>" data-parent="chk_all" data-validate="gsite_ids" class="form-check-input gsite_id">
                                    <label for="gsite_id_<?php echo $value['gsite_id']; ?>" class="form-check-label">
                            ...
                            <td>
                                ...
                                <div class="mb-2 text-wrap text-break">
                                    <a href="<?php echo $route_console; ?>gsite/form/id/<?php echo $value['gsite_id']; ?>/">
                                ...
                                <div class="bg-manage-menu">
                                    <div class="d-flex flex-wrap">
                                        ...
                                        <a href="<?php echo $route_console; ?>gsite/form/id/<?php echo $value['gsite_id']; ?>/" class="mr-2">
                                            <span class="fas fa-edit"></span>
                                            <?php echo $lang->get('Edit'); ?>
                                        </a>
                                        <a href="javascript:void(0);" data-id="<?php echo $value['gsite_id']; ?>" class="gsite_delete text-danger">
        ...
        <div class="mb-3">
            <small class="form-text" id="msg_gsite_ids"></small>
        </div>
...
<?php include($cfg['pathInclude'] . 'console_foot' . GK_EXT_TPL); ?>
    ...
    var opts_validate_list = {
        ...
        attr_names: {
            gsite_ids: '<?php echo $lang->get('Gathering site'); ?>',
    ...
    $(document).ready(function(){
        ...
        var obj_validate_list  = $('#gsite_list').baigoValidate(opts_validate_list);
        var obj_submit_list     = $('#gsite_list').baigoSubmit(opts_submit_list);

        $('#gsite_list').submit(function(){
            var _act = $('#act').val();
                ...
                switch (_act) {
                    case 'delete':
                        obj_dialog.confirm('<?php echo $lang->get('Are you sure to delete?'); ?>', function(confirm_result){
                            if (confirm_result) {
                                obj_submit_list.formSubmit('<?php echo $route_console; ?>gsite/delete/');
                            }
                        });
                    break;

                    default:
                        obj_submit_list.formSubmit('<?php echo $route_console; ?>gsite/status/');
                    break;
                }
            }
        });

        $('.gsite_delete').click(function(){
            var _gsite_id = $(this).data('id');
