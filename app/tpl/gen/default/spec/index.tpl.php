...
include($tpl_include . 'html_head' . GK_EXT_TPL); ?>
  ...
  <div id="modal_body">
    <div class="table-responsive" id="modal_body">
      <table class="table">
        <thead>
          <tr>
            <th>
...
<?php include($tpl_include . 'script_foot' . GK_EXT_TPL); ?>
  ...
  $(document).ready(function(){
    ...
    <?php for ($_iii = 1; $_iii <= $pageRow['total']; $_iii++) { ?>
      $.ajax({
        url: '<?php echo $hrefRow['lists']; ?>?' + new Date().getTime() + '=' + Math.random(), //url
        //async: false, //设置为同步
        type: 'post',
        dataType: 'json',
        data: {
        ...
        error: function (result) {
          ...
          $('#spec_<?php echo $_iii; ?> .bg-icon').html('<?php include($tpl_icon . 'times-circle' . BG_EXT_SVG); ?>');
        ...
        success: function(_result){ //读取返回结果
          ...
          $('#spec_<?php echo $_iii; ?> .bg-icon').html(_icon);
