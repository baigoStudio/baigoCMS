...
include_once(BG_PATH_CONTROL_PUB . "pub/article.class.php"); //载入文章类
		...
		if ($arr_articleRow["str_alert"] != "y120102") {
			if ($arr_articleRow["cate_link"]) {
				...
				if ($arr_articleRow["article_link"]) {
					...
					$_str_linkUrl = BG_URL_PUB . "index.php?mod=alert&act_get=display&alert=" . $arr_articleRow["str_alert"];
				}
			}
