...
class CONTROL_SPEC {
	...
	function ctl_show() {
		...
		$_num_articleCount    = $this->mdl_articlePub->mdl_count("", "", "", false, false, $_num_specId);
		$_arr_page            = fn_page($_num_articleCount, BG_SITE_PERPAGE); //取得分页数据
		$_str_query           = http_build_query($this->search);
		$_arr_articleRows     = $this->mdl_articlePub->mdl_list(BG_SITE_PERPAGE, $_arr_page["except"], "", "", "", false, false, $_num_specId);
