...
class CONTROL_PUB_UI_ARTICLE {
    ...
    function ctrl_show() {
        ...
        if (!isset($this->articleRow["article_title"]) || fn_isEmpty($this->articleRow["article_title"]) || $this->articleRow["article_status"] != "pub" || $this->articleRow["article_box"] != "normal" || $this->articleRow["article_time_pub"] > time() || ($this->articleRow["article_time_hide"] > 0 && $this->articleRow["article_time_hide"] < time())) {
            $_arr_tplData = array(
                "rcode" => "x120102",
            );
            $this->obj_tpl->tplDisplay("error", $_arr_tplData);
