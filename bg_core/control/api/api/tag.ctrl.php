...
class CONTROL_API_API_TAG {
    ...
    function ctrl_read() {
        ...
        $_arr_pluginReturn = $GLOBALS['obj_plugin']->trigger('filter_api_tag_read', $_arr_tagRow); //编辑文章时触发
        if (isset($_arr_pluginReturn['filter_api_tag_read'])) {
            $_arr_pluginReturnDo    = $_arr_pluginReturn['filter_api_tag_read'];

            if (isset($_arr_pluginReturnDo['return']) && !fn_isEmpty($_arr_pluginReturnDo['return'])) { //如果有插件返回
                $_arr_tagRow = $_arr_pluginReturnDo['return'];
            }
        }
