...
class CONTROL_ADMIN {
	...
	function ctl_my() {
		...
		$this->obj_tpl->tplDisplay("admin_my.tpl", $_arr_tplData);
	 ...
	 */
	function ctl_show() {
		if ($this->adminLogged["admin_allow_sys"]["admin"]["browse"] != 1) {
			return array(
				"str_alert" => "x020301",
			);
		}
		...
		$_arr_groupRow["group_allow"]         = json_decode($_arr_groupRow["group_allow"]); //json解码
	...
	function ctl_list() {
		...
		foreach ($_arr_adminRows as $_key=>$_value) {
			...
			$_arr_adminRows[$_key]["group_name"] = $_arr_groupRow["group_name"];
		}
