...
class CONTROL_CONSOLE_REQUEST_CATE {
    ...
    function ctrl_submit() {
        ...
        if ($_arr_cateInput['cate_id'] > 0) {
            ...
            $_arr_pluginReturn = $GLOBALS['obj_plugin']->trigger('filter_console_cate_edit', $_arr_cateInput); //编辑文章时触发
            if (isset($_arr_pluginReturn['filter_console_cate_edit'])) {
                $_arr_pluginReturnDo    = $_arr_pluginReturn['filter_console_cate_edit'];
            }
        } else {
            if (!isset($this->group_allow['cate']['add']) && !$this->is_super) {
                $_arr_tplData = array(
            ...
            $_arr_pluginReturn = $GLOBALS['obj_plugin']->trigger('filter_console_cate_add', $_arr_cateInput); //编辑文章时触发
            if (isset($_arr_pluginReturn['filter_console_cate_add'])) {
                $_arr_pluginReturnDo    = $_arr_pluginReturn['filter_console_cate_add'];
            }
        }

        if (isset($_arr_pluginReturnDo['return']) && !fn_isEmpty($_arr_pluginReturnDo['return'])) { //如果有插件返回
            $_arr_pluginInput = $this->mdl_cate->input_submit($_arr_pluginReturnDo['return']);
    ...
    function ctrl_status() {
        ...
        $_arr_return = array(
            ...
            'cate_status'   => $_str_status,
        );

        $GLOBALS['obj_plugin']->trigger('action_console_cate_status', $_arr_return); //删除链接时触发
        ...
        if (defined('BG_MODULE_GEN') && BG_MODULE_GEN > 0 && defined('BG_VISIT_TYPE') && BG_VISIT_TYPE == 'static') {
            if ($_str_status != 'show') {
                ...
                foreach ($_arr_cateRows as $_key=>$_value) {
                    $this->obj_file->dir_del($_value['urlRow']['cate_path']);
                    if (defined('BG_MODULE_FTP') && BG_MODULE_FTP > 0) {
                        if ($_value['cate_parent_id'] == 0 && isset($_value['cate_ftp_host']) && !fn_isEmpty($_value['cate_ftp_host'])) {
                            ...
                            $this->obj_ftp->dir_del($_value['cate_ftp_path'] . $_value['urlRow']['cate_pathShort']);
                        }
                    }
                }
            }
    ...
    function ctrl_del() {
        ...
        $GLOBALS['obj_plugin']->trigger('action_console_cate_del', $_arr_cateIds['cate_ids']); //删除链接时触发
