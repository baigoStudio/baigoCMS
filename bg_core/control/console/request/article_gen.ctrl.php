...
class CONTROL_CONSOLE_REQUEST_ARTICLE_GEN {
     ...
     * @access public
     */
    function ctrl_single($num_articleId = 0) {
        if ($num_articleId > 0) {
    ...
    private function output_process($arr_articleRow) {
        ...
        $_arr_pluginReturn = $GLOBALS['obj_plugin']->trigger('filter_gen_article_show', $_arr_tplData); //编辑文章时触发
        if (isset($_arr_pluginReturn['filter_gen_article_show'])) {
            $_arr_pluginReturnDo    = $_arr_pluginReturn['filter_gen_article_show'];
        ...
        $_num_size      = $this->obj_file->file_put($arr_articleRow['urlRow']['article_path'] . $arr_articleRow['article_id'] . $arr_articleRow['urlRow']['page_ext'], $_str_outPut);
        ...
        if (BG_MODULE_FTP > 0 && isset($this->ftpRow['cate_ftp_host']) && !fn_isEmpty($this->ftpRow['cate_ftp_host'])) {
            ...
            $_ftp_status = $this->obj_ftp->file_upload($arr_articleRow['urlRow']['article_pathFull'], $this->ftpRow['cate_ftp_path'] . $arr_articleRow['urlRow']['article_pathShort']);
        }
    }
}
