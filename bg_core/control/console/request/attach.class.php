...
class CONTROL_CONSOLE_REQUEST_ATTACH {
    ...
    function ctrl_gen() {
        ...
        $_str_msg    = $this->obj_tpl->rcode["y070409"];
        ...
        if ($_arr_page["page"] < $_arr_page["total"]) {
            ...
            $_str_status = "loading";
            $_str_msg    = $this->obj_tpl->rcode["x070409"];
    ...
    function ctrl_empty() {
        ...
        if ($_num_attachCount > 0) {
            ...
            $_str_status     = "loading";
            $_str_msg        = $this->obj_tpl->rcode["x070408"];
        } else {
            $_str_status = "complete";
            $_str_msg    = $this->obj_tpl->rcode["y070408"];
        }
    ...
    function ctrl_clear() {
        ...
        if (fn_isEmpty($_arr_attachRows)) {
            $_str_status    = "complete";
            $_str_msg       = $this->obj_tpl->rcode["y070407"];
        } else {
            foreach ($_arr_attachRows as $_key=>$_value) {
            ...
            $_str_status    = "loading";
            $_str_msg       = $this->obj_tpl->rcode["x070407"];
    ...
    function ctrl_box() {
        ...
        $_str_attachStatus = fn_getSafe($GLOBALS["act"], "txt", "");
    ...
    function ctrl_submit() {
        ...
        if ($_arr_uploadRow["rcode"] != "y100201") {
            if ($_arr_uploadRow["rcode"] == "x070203") {
                $_arr_uploadRow["msg"] = $this->obj_tpl->rcode[$_arr_uploadRow["rcode"]] . " " . BG_UPLOAD_SIZE . " " . BG_UPLOAD_UNIT;
            }
