...
class CONTROL_CONSOLE_REQUEST_CALL {
    ...
    function ctrl_status() {
        ...
        $_arr_callRow = $this->mdl_call->mdl_status($_str_status);
    ...
    function ctrl_duplicate() {
        $_arr_callInput = $this->mdl_call->input_duplicate();
        if ($_arr_callInput['rcode'] != 'ok') {
            $this->obj_tpl->tplDisplay('result', $_arr_callInput);
        }

        if (!isset($this->group_allow['gather']['call']) && !$this->is_super) {
            $_arr_tplData = array(
                'rcode' => 'x170302',
            );
            $this->obj_tpl->tplDisplay('result', $_arr_tplData);
        ...
        $_arr_callRow = $this->mdl_call->mdl_duplicate();
