...
class CONTROL_CONSOLE_REQUEST_LINK {
    ...
    function ctrl_status() {
        ...
        $_arr_return = array(
            'link_ids'      => $_arr_linkIds['link_ids'],
            'link_status'   => $_str_status,
        );

        $GLOBALS['obj_plugin']->trigger('action_console_link_status', $_arr_return); //删除链接时触发
    ...
    function ctrl_submit() {
        ...
        $_arr_linkInput['link_url'] = fn_htmlcode($_arr_linkInput['link_url'], 'decode', 'url');
        ...
        if ($_arr_linkInput['link_id'] > 0) {
            ...
            $_arr_pluginReturn = $GLOBALS['obj_plugin']->trigger('filter_console_link_edit', $_arr_linkInput); //编辑链接时触发

            if (isset($_arr_pluginReturn['filter_console_link_edit'])) {
                $_arr_pluginReturnDo    = $_arr_pluginReturn['filter_console_link_edit'];
            }
        } else {
            ...
            $_arr_pluginReturn = $GLOBALS['obj_plugin']->trigger('filter_console_link_add', $_arr_linkInput); //添加链接时触发

            if (isset($_arr_pluginReturn['filter_console_link_add'])) {
                $_arr_pluginReturnDo    = $_arr_pluginReturn['filter_console_link_add'];
            }
        }

        if (isset($_arr_pluginReturnDo['return']) && !fn_isEmpty($_arr_pluginReturnDo['return'])) { //如果有插件返回
            $_arr_pluginInput = $this->mdl_link->input_submit($_arr_pluginReturnDo['return']);
    ...
    function ctrl_del() {
        ...
        $GLOBALS['obj_plugin']->trigger('action_console_link_del', $_arr_linkIds['link_ids']); //删除链接时触发
