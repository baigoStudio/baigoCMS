...
class CONTROL_CONSOLE_REQUEST_ARTICLE {
    ...
    function ctrl_submit() {
        ...
        //验证表单
        $_arr_articleInput = $this->mdl_article->input_submit($_num_excerptCount);
        ...
        if (!fn_isEmpty($_arr_qlistImgs)) {
            ...
            $_arr_articleSubmit = array(
                'article_content'   => $_arr_articleInput['article_content'],
                'article_attach_id' => $_arr_returnRows[0]['attach_id'],
            );
        }
        ...
        if (defined('BG_MODULE_GEN') && BG_MODULE_GEN > 0 && defined('BG_VISIT_TYPE') && BG_VISIT_TYPE == 'static') {
            if ($_arr_articleRow['article_id'] > 0) {
                $this->ctrl_article_gen->ctrl_single($_arr_articleRow['article_id'], false);
            }
        }

        if ($_arr_articleRow['rcode'] == 'y120101' || $_arr_articleRow['rcode'] == 'y120103') {
            fn_cookie('post_check_article', 'mk', $_str_postChkMd5);
        }
    ...
    function ctrl_box() {
        ...
        $_str_articleBox = fn_getSafe($GLOBALS['route']['bg_act'], 'txt', '');
        ...
        $_arr_articleRow = $this->mdl_article->mdl_box($_str_articleBox, $_arr_cateIds, $_num_adminId);
