...
class CONTROL_CONSOLE_UI_CATE {
    ...
    function __construct() { //构造函数
        ...
        $this->tplData = array(
            ...
            'status'        => $this->mdl_cate->arr_status,
            'type'          => $this->mdl_cate->arr_type,
        );

        if (BG_MODULE_FTP > 0) {
            $this->obj_ftp  = new CLASS_FTP(); //设置 FTP 对象
            $this->tplData['pasv'] = $this->obj_ftp->arr_pasv;
        }
    }
    ...
    function ctrl_form() {
        ...
        if ($_num_cateId > 0) {
            ...
            $_arr_cateRow = array(
                ...
                'cate_content'      => '',
                'cate_link'         => '',
                'cate_type'         => $this->mdl_cate->arr_type[0],
        ...
        $_arr_tplRows     = $this->obj_file->dir_list(BG_PATH_TPL . 'pub/');
