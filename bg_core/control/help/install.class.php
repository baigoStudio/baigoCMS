...
class CONTROL_INSTALL {
	...
	function __construct() { //构造函数
		...
		$this->tplData = array(
			...
			"config"     => include_once(BG_PATH_LANG . $this->config["lang"] . "/help/install/config.php"),
			"active"     => "install",
			"mod"        => "install",
		);
	}
	...
	private function str_process($str_load) {
		$_str_content = include_once(BG_PATH_LANG . $this->config["lang"] . "/help/install/" . $str_load . ".php");
		$_str_content = str_replace("{images}", BG_URL_STATIC_HELP . $this->config["ui"] . "/install/", $_str_content);
