...
class CONTROL_UPGRADE {
	...
	function __construct() { //构造函数
		...
		$this->tplData = array(
			...
			"config"     => include_once(BG_PATH_LANG . $this->config["lang"] . "/help/upgrade/config.php"),
			"active"     => "install",
			"mod"        => "upgrade",
		);
	}
	...
	private function str_process($str_load) {
		$_str_content = include_once(BG_PATH_LANG . $this->config["lang"] . "/help/upgrade/" . $str_load . ".php");
		$_str_content = str_replace("{images}", BG_URL_STATIC_HELP . $this->config["ui"] . "/upgrade/", $_str_content);
