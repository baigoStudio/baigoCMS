...
ok 正常
*/
function validateLeng($str, $min, $max) {
...
@format 格式，text 为任意
*/
function validateReg($str, $format) {
	switch ($format) {
		case "date":
...
@format 格式
*/
function isText($str, $min, $max, $format) {
	$_status_leng = validateLeng($str, $min, $max);
	if ($_status_leng != "ok") {
		$_status = $_status_leng; //如验证长度出错，直接返回错误
	} else {
		if (validateReg($str, $format)) {
...
*/
function isDigit($num, $min, $max, $format) {
	if (validateReg($num, $format)) {
		if ($min > 0 && $num < $min ){
...
function validateStr($str, $min, $max, $type = "str", $format = "text") {
	...
	switch ($type) {
		case "str":
			$_status = isText($str, $min, $max, $format); //验证字符串
		break;
		case "digit":
			$_status = isDigit($str, $min, $max, $format); //验证字符串
		break;
		case "num":
			$_status = isNum($str, $min, $max); //验证个数
		break;
	}
