...
{include "include/install_head.tpl" cfg=$cfg}
	...
	<div class="page_head">
		...
		&raquo;
		{$lang.page.installUpfile}
	</div>

	<div class="page_body">
		<form name="instal_form_upfile" id="instal_form_upfile">
...
{include "include/install_foot.tpl" cfg=$cfg}
	...
	var opts_submit_form = { ajax_url: "{$smarty.const.BG_URL_INSTALL}ajax.php?mod=install", btn_text: "{$lang.btn.installNext}", btn_url: "{$smarty.const.BG_URL_INSTALL}install.php?mod=install&act_get=sso" };

	$(document).ready(function(){
		var obj_validator_form = $("#instal_form_upfile").baigoValidator(opts_validator_form);
		var obj_submit_form = $("#instal_form_upfile").baigoSubmit(opts_submit_form);
