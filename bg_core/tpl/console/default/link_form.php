                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <small class="form-text" id="msg_link_cate_id"></small>
                        </div>
                            ...
                            <?php foreach ($this->tplData['type'] as $key=>$value) { ?>
                                <div class="form-check">
                                    <label for="link_type_<?php echo $value; ?>" class="form-check-label">
                                        <input type="radio" name="link_type" id="link_type_<?php echo $value; ?>" value="<?php echo $value; ?>" <?php if ($this->tplData['linkRow']['link_type'] == $value) { ?>checked<?php } ?> data-validate="link_type" class="form-check-input">
...
<?php include($cfg['pathInclude'] . 'console_foot.php'); ?>
    ...
    var opts_validator_form = {
        link_name: {
            ...
            msg: { too_short: "<?php echo $this->lang['rcode']['x240201']; ?>", too_long: "<?php echo $this->lang['rcode']['x240202']; ?>" }
        },
        link_url: {
            len: { min: 1, max: 900 },
            validate: { type: "str", format: "url" },
            msg: { too_short: "<?php echo $this->lang['rcode']['x240203']; ?>", too_long: "<?php echo $this->lang['rcode']['x240204']; ?>", format_err: "<?php echo $this->lang['rcode']['x240205']; ?>" }
        },
        link_type: {
            len: { min: 1, max: 0 },
            validate: { selector: "input[name='link_type']", type: "radio" },
            msg: { too_few: "<?php echo $this->lang['rcode']['x240206']; ?>" }
        },
        link_status: {
            len: { min: 1, max: 0 },
            validate: { selector: "input[name='link_status']", type: "radio" },
            msg: { too_few: "<?php echo $this->lang['rcode']['x240207']; ?>" }
        },
        link_cate_id: {
            len: { min: 1, max: 0 },
            validate: { type: "select" },
            msg: { too_few: "<?php echo $this->lang['rcode']['x240208']; ?>" }
        }
    };
