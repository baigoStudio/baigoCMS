<?php $cfg = array(
    "title"          => $this->consoleMod["cate"]["main"]["title"],
    "menu_active"    => "cate",
    "sub_active"     => "list",
    "baigoCheckall"  => "true",
    "baigoSubmit"    => "true",
...
function cate_list_table($arr_cateRows, $status_cate, $type_cate, $lang = array()) {
    foreach ($arr_cateRows as $key=>$value) { ?>
        <tr<?php if ($value["cate_level"] == 1) { ?> class="active"<?php } ?>>
            ...
            <td class="bg-child-" . $value["cate_level"] . "">
                    ...
                    <li>
                        <ul class="bg-nav-line">
                            ...
                            <?php if (BG_MODULE_GEN > 0 && BG_VISIT_TYPE == "static" && $value["cate_status"] == "show") { ?>
                                <li>
                                    <a class="btn btn-xs btn-info" data-url="<?php echo BG_URL_CONSOLE; ?>gen.php?mod=cate&act=single&cate_id=<?php echo $value["cate_id"]; ?>" data-toggle="modal" href="#gen_modal">
                                        <span class="glyphicon glyphicon-refresh"></span>
                                        <?php echo $lang["btn"]["cateGenSingle"]; ?>
                                    </a>
                                </li>
                            <?php } ?>
                        </ul>
...
include($cfg["pathInclude"] . "console_head.php"); ?>
    ...
    <?php if (BG_MODULE_GEN > 0 && BG_VISIT_TYPE == "static") { ?>
        <div class="form-group">
            <button data-url="<?php echo BG_URL_CONSOLE; ?>gen.php?mod=cate&act=1by1" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#gen_modal">
                <span class="glyphicon glyphicon-refresh"></span>
                <?php echo $this->lang["btn"]["cateGen1by1"]; ?>
            </button>
        </div>
    <?php } ?>
                ...
                <table class="table table-hover">
                    <thead>
                        <tr>
                            ...
                            <th><?php echo $this->lang["label"]["cateName"]; ?></th>
                            <th><?php echo $this->lang["label"]["cateAlias"]; ?></th>
                            <th class="text-nowrap bg-td-sm"><?php echo $this->lang["label"]["status"]; ?> / <?php echo $this->lang["label"]["type"]; ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php cate_list_table($this->tplData["cateRows"], $this->status["cate"], $this->type["cate"], $this->lang); ?>
                    </tbody>
                    <tfoot>
                        <tr>
...
<?php include($cfg["pathInclude"] . "console_foot.php"); ?>
    ...
    var opts_validator_list = {
        cate_id: {
            ...
            msg: { selector: "#msg_cate_id", too_few: "<?php echo $this->rcode["x030202"]; ?>" }
        },
        act: {
            len: { min: 1, max: 0 },
    ...
    $(document).ready(function(){
        $("#cate_modal").on("shown.bs.modal",function(event){
            ...
            $("#cate_modal .modal-content").load("<?php echo BG_URL_CONSOLE; ?>index.php?mod=cate&act=order&cate_id=" + _id + "&view=iframe");
        });
