...
{include "{$smarty.const.BG_PATH_TPLSYS}admin/default/include/admin_foot.tpl" cfg=$cfg}
    ...
    var opts_validator_form = {
        app_name: {
            ...
            msg: { selector: "#msg_app_name", too_short: "{$alert.x190201}", too_long: "{$alert.x190202}" }
        },
        app_ip_allow: {
            len: { min: 0, max: 3000 },
            validate: { type: "str", format: "text", group: "#group_app_ip_allow" },
            msg: { selector: "#msg_app_ip_allow", too_long: "{$alert.x190210}" }
        },
        app_ip_bad: {
            len: { min: 0, max: 3000 },
            validate: { type: "str", format: "text", group: "#group_app_ip_bad" },
            msg: { selector: "#msg_app_ip_bad", too_long: "{$alert.x190211}" }
        },
        app_note: {
            len: { min: 0, max: 30 },
            validate: { type: "str", format: "text", group: "#group_app_note" },
            msg: { selector: "#msg_app_note", too_long: "{$alert.x190205}" }
        },
        app_status: {
            len: { min: 1, max: 0 },
            validate: { selector: "input[name='app_status']", type: "radio", group: "#group_app_status" },
            msg: { selector: "#msg_app_status", too_few: "{$alert.x190206}" }
        }
    };
