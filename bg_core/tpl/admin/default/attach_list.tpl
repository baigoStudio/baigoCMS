{$cfg = [
    ...
    tokenReload    => "true",
    tooltip        => "true",
    str_url        => "{$smarty.const.BG_URL_ADMIN}ctl.php?mod=attach&act_get=list&{$tplData.query}"
]}
                        ...
                        <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                {foreach $tplData.attachRows as $key=>$value}
                                    ...
                                    {/if}
                                    <tr>
                                        <td class="text-nowrap td_mn"><input type="checkbox" name="attach_ids[]" value="{$value.attach_id}" id="attach_id_{$value.attach_id}" data-validate="attach_id" data-parent="chk_all"></td>
...
{include "{$smarty.const.BG_PATH_TPLSYS}admin/default/include/admin_foot.tpl" cfg=$cfg}
    ...
    var opts_validator_list = {
        attach_id: {
            ...
            msg: { selector: "#msg_attach_id", too_few: "{$alert.x030202}" }
        },
        act_post: {
            len: { min: 1, "max": 0 },
