                    ...
                    <div class="panel-body">
                        <div class="form-group">
                            <div id="group_spec_name">
                                <label class="control-label">{$lang.label.specName}<span id="msg_spec_name">*</span></label>
                                <input type="text" name="spec_name" id="spec_name" value="{$tplData.specRow.spec_name}" data-validate class="form-control">
                            </div>
                        </div>

                        <label class="control-label">{$lang.label.specContent}</label>
                        <div class="form-group">
                            <a href="{$smarty.const.BG_URL_ADMIN}ctl.php?mod=attach&act_get=form&view=iframe" class="btn btn-success btn-sm" data-toggle="modal" data-target="#spec_modal">
                            ...
                            {foreach $status.spec as $key=>$value}
                                <div class="radio_baigo">
                                    <label for="spec_status_{$key}">
                                        <input type="radio" name="spec_status" id="spec_status_{$key}" {if $tplData.specRow.spec_status == $key}checked{/if} value="{$key}" data-validate="spec_status">
                                        {$value}
                                    </label>
                                </div>
                            {/foreach}
...
{include "{$smarty.const.BG_PATH_TPLSYS}admin/default/include/admin_foot.tpl" cfg=$cfg}
    ...
    var opts_validator_form = {
        spec_name: {
            ...
            msg: { selector: "#msg_spec_name", too_short: "{$alert.x180201}", too_long: "{$alert.x180202}" }
        },
        spec_content: {
            len: { min: 0, max: 3000 },
            validate: { type: "str", format: "text" },
            msg: { selector: "#msg_spec_content", too_long: "{$alert.x180203}" }
        },
        spec_status: {
            len: { min: 1, max: 0 },
            validate: { selector: "input[name='spec_status']", type: "radio", group: "#group_spec_status" },
            msg: { selector: "#msg_spec_status", too_few: "{$alert.x180205}" }
        }
    };
    var opts_submit_form = {
        ...
        btn_url: "{$smarty.const.BG_URL_ADMIN}ctl.php?mod=spec&act_get=select",
        attach_key: "spec_id"
    };

    $(document).ready(function(){
        $("#spec_modal").on("hidden.bs.modal", function() {
