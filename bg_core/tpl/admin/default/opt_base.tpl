...
{include "include/admin_head.tpl" cfg=$cfg}
	...
	<form name="opt_form" id="opt_form" class="tform">
		...
		<ul>
			...
			<li class="title">{$lang.label.tpl}<span id="msg_BG_SITE_TPL">*</span></li>
			<li class="field">
				<select name="opt[BG_SITE_TPL]" id="BG_SITE_TPL" class="validate">
					{foreach $tplData.tplRows as $key=>$value}
						{if $value["type"] == "dir"}
						<option {if $tplData.optRows.BG_SITE_TPL.opt_value == $value.name}selected="selected"{/if} value="{$value.name}">{$value.name}</option>
						{/if}
					{/foreach}
				</select>
			</li>
