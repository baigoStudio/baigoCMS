{$cfg = [
    ...
    baigoSubmit    => "true",
    tokenReload    => "true",
    str_url        => "{$smarty.const.BG_URL_ADMIN}ctl.php?mod=group&act_get=list&{$tplData.query}"
]}
                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        {foreach $tplData.groupRows as $key=>$value}
                            ...
                            {/if}
                            <tr>
                                <td class="text-nowrap td_mn"><input type="checkbox" name="group_ids[]" value="{$value.group_id}" id="group_id_{$value.group_id}" data-parent="chk_all" data-validate="group_id"></td>
                                ...
                                <td class="td_sm label_baigo">
                                    <span class="label label-{$css_status}">{$status.group[$value.group_status]}</span>
                                </td>
                            </tr>
                        {/foreach}
                    </tbody>
...
{include "{$smarty.const.BG_PATH_TPLSYS}admin/default/include/admin_foot.tpl" cfg=$cfg}
    ...
    var opts_validator_list = {
        group_id: {
            ...
            msg: { selector: "#msg_group_id", too_few: "{$alert.x030202}" }
        },
        act_post: {
            len: { min: 1, max: 0 },
