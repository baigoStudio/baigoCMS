...
class MODEL_ADMIN {
    ...
    function mdl_alter_table() {
        ...
        if (in_array("admin_group_id", $_arr_col)) {
            $_arr_alter["admin_group_id"] = array("CHANGE", "smallint NOT NULL COMMENT '从属用户组ID'", "admin_group_id");
        }
        ...
        if (!in_array("admin_type", $_arr_col)) {
            $_arr_alter["admin_type"] = array("ADD", "enum('" . $_str_types . "') NOT NULL COMMENT '类型'");
        }
    ...
    function mdl_login($num_adminId, $str_accessToken, $tm_accessExpire, $str_refreshToken, $tm_refreshExpire) {
        ...
        return array(
            "admin_ip"          => $_str_adminIp, //更新成功
            "admin_time_login"  => $_tm_timeLogin, //更新成功
            "rcode"             => $_str_rcode, //更新成功
        );
    }
    ...
    function input_submit() {
        ...
        $this->adminInput["admin_allow_cate"]      = fn_jsonEncode(fn_post("admin_allow_cate"), "no");
