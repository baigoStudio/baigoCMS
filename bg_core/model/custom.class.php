...
class MODEL_CUSTOM {
    ...
    function mdl_create_index() {
        ...
        $_num_db = $this->obj_db->create_index('order', BG_DB_TABLE . 'custom', $_arr_customIndex, 'BTREE', $is_exists);
    ...
    function mdl_cache($is_reGen = false) {
        if ($is_reGen || !file_exists(BG_PATH_CACHE . 'sys' . DS . 'custom_list.json')) {
            ...
            $_num_size = $this->obj_dir->put_file(BG_PATH_CACHE . 'sys' . DS . 'custom_list.json', $_str_outPut);
        }

        $_str_cacheReturn = file_get_contents(BG_PATH_CACHE . 'sys' . DS . 'custom_list.json');
