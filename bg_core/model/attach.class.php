...
class MODEL_ATTACH {
	...
	function mdl_create_table() {
		$_arr_attachCreat = array(
			...
			"attach_mime"        => "varchar(30) NOT NULL COMMENT 'MIME'",
	 ...
	 * @return void
	 */
	function mdl_list($num_no, $num_except = 0, $str_key = "", $str_year = "", $str_month = "", $str_ext = "", $num_adminId = 0, $str_box = "normal", $arr_attachIds = false, $num_begin = 0, $num_end = 0) {
		$_arr_attachSelect = array(
			"attach_id",
			"attach_name",
			"attach_time",
		...
		if ($arr_attachIds) {
			$_str_attachIds  = implode(",", $arr_attachIds);
			$_str_sqlWhere  .= " AND attach_id IN (" . $_str_attachIds . ")";
		}

		if ($num_begin > 0) {
			$_str_sqlWhere .= " AND attach_id>" . $num_begin;
		}

		if ($num_end > 0) {
			$_str_sqlWhere .= " AND attach_id<" . $num_end;
		}
	 ...
	 * @return void
	 */
	function mdl_count($str_key = "", $str_year = "", $str_month = "", $str_ext = "", $num_adminId = 0, $str_box = "normal", $arr_attachIds = false, $num_begin = 0, $num_end = 0) {
		$_str_sqlWhere = "1=1";
