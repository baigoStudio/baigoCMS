...
class MODEL_CATE_BELONG {
    ...
    function mdl_create_table() {
        ...
        $_num_db = $this->obj_db->create_table(BG_DB_TABLE . 'cate_belong', $_arr_belongCreat, 'belong_id', '栏目从属');
    ...
    function mdl_create_index() {
        $_arr_indexRow    = $this->obj_db->show_indexs(BG_DB_TABLE . 'cate_belong');
        $drop_overlap        = false;
        ...
        $_num_db = $this->obj_db->create_index('search', BG_DB_TABLE . 'cate_belong', $_arr_tagBelongIndex, 'BTREE', $drop_overlap);
        ...
        if ($num_articleId > 0 && $num_cateId > 0) { //插入
            ...
            if ($_arr_belongRow['rcode'] == 'x150102') { //插入
                if ($_arr_belongRow['rcode'] == 'x150102') { //插入
                    ...
                    if ($_arr_belongRow['rcode'] == 'y150102') {
                        $_num_db = $this->obj_db->update(BG_DB_TABLE . 'cate_belong', $_arr_belongData, '`belong_id`=' . $_arr_belongRow['belong_id']); //删除数据
                        if ($_num_db > 0) {
                            $_str_rcode = 'y150103';
                        } else {
                            $_str_rcode = 'x150103';
    ...
    function mdl_read($num_articleId = 0, $num_cateId = 0) {
        ...
        $_str_sqlWhere = '`belong_cate_id`=' . $num_cateId;
    ...
    function mdl_del($num_cateId = 0, $num_articleId = 0, $arr_cateIds = false, $arr_articleIds = false, $arr_notCateIds = false, $arr_notArticleIds = false) {
        ...
        $_num_db = $this->obj_db->update(BG_DB_TABLE . 'cate_belong', $_arr_belongData, $_str_sqlWhere); //删除数据
