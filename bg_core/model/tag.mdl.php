...
class MODEL_TAG {
    ...
    function mdl_create_index() {
        $_arr_indexRow    = $this->obj_db->show_indexs(BG_DB_TABLE . 'tag');
        $drop_overlap        = false;
        ...
        $_num_db = $this->obj_db->create_index('search', BG_DB_TABLE . 'tag', $_arr_tagIndex, 'BTREE', $drop_overlap);
    ...
    function mdl_submit() {
        ...
        if (isset($this->tagInput['tag_name'])) {
            $_arr_tagData['tag_name'] = $this->tagInput['tag_name'];
        }

        if (isset($this->tagInput['tag_status'])) {
    ...
    private function url_process($_arr_tagRow) {
        ...
        switch (BG_VISIT_TYPE) {
            ...
            default:
                $_str_tagUrl        = BG_URL_ROOT . 'index.php?m=tag&a=show&tag_name=' . urlencode($_arr_tagRow['tag_name']);
                $_str_pageAttach    = '&page=';
            break;
        }
