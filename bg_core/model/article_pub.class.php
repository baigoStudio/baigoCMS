...
class MODEL_ARTICLE_PUB {
    ...
    function mdl_create_cate_view() {
        $_arr_articleCreat = array(
            ...
            array('article_time_show', BG_DB_TABLE . 'article'),
            array('article_is_time_pub', BG_DB_TABLE . 'article'),
            array('article_time_pub', BG_DB_TABLE . 'article'),
            array('article_is_time_hide', BG_DB_TABLE . 'article'),
        ...
        $_num_db = $this->obj_db->create_view(BG_DB_TABLE . 'article_cate_view', $_arr_articleCreat, BG_DB_TABLE . 'article', $_str_sqlJoin);

        if ($_num_db > 0) {
            $_str_rcode = 'y150108'; //更新成功
        } else {
            $_str_rcode = 'x150108'; //更新成功
        }
    ...
    function mdl_create_tag_view() {
        ...
        $_num_db = $this->obj_db->create_view(BG_DB_TABLE . 'article_tag_view', $_arr_articleCreat, BG_DB_TABLE . 'article', $_str_sqlJoin);
    ...
    function mdl_create_custom_view($arr_customRows) {
        ...
        $_num_db = $this->obj_db->create_view(BG_DB_TABLE . 'article_custom_view', $_arr_articleCreat, BG_DB_TABLE . 'article', $_str_sqlJoin);

        if ($_num_db > 0) {
            $_str_rcode = 'y210108'; //更新成功
        } else {
            $_str_rcode = 'x210108'; //更新成功
        }
    ...
    function mdl_create_spec_view() {
        ...
        $_num_db = $this->obj_db->create_view(BG_DB_TABLE . 'article_spec_view', $_arr_articleCreat, BG_DB_TABLE . 'article', $_str_sqlJoin);
    ...
    function mdl_read($num_articleId, $not_gen = false, $is_max = false) {
        ...
        $_str_sqlWhere = '`article_status`=\'pub\' AND `article_box`=\'normal\' AND (`article_is_time_pub`<1 OR `article_time_pub`<=' . time() . ') AND (`article_is_time_hide`<1 OR `article_time_hide`>' . time() . ')';
        ...
        if (isset($_arr_contentRows[0])) {
            ...
            if (fn_isEmpty($_arr_contentRow['article_content'])) {
                ...
                $_arr_articleRow['article_content'] = fn_htmlcode(stripslashes($_arr_contentRow['article_content']), 'decode');
            }
            ...
            $_arr_articleRow['article_source_url']  = fn_htmlcode($_arr_contentRow['article_source_url'], 'decode', 'url');
            $_arr_articleRow['article_source_url']  = fn_safe($_arr_articleRow['article_source_url']);
            $_arr_articleRow['article_source_url']  = fn_htmlcode($_arr_articleRow['article_source_url'], 'decode', 'url');
        } else {
            $_arr_articleRow['article_content']     = '';
        ...
        $_arr_articleRow['article_excerpt'] = fn_htmlcode(stripslashes($_arr_articleRow['article_excerpt']), 'decode');
    ...
    function mdl_list($num_no, $num_except = 0, $arr_search = array(), $str_orderType = '') {
        ...
        if (isset($arr_search['tag_ids']) && !fn_isEmpty($arr_search['tag_ids'])) {
            $_view_name = 'tag';
        } else if (isset($arr_search['spec_ids']) && !fn_isEmpty($arr_search['spec_ids'])) {
            $_view_name = 'spec';
        } else if (isset($arr_search['custom_rows']) && !fn_isEmpty($arr_search['custom_rows'])) {
            $_view_name = 'custom';
        } else {
            $_view_name = 'cate';
        }
    ...
    private function sql_process($arr_search = array()) {
        ...
        if (isset($arr_search['mark_ids']) && !fn_isEmpty($arr_search['mark_ids'])) {
        ...
        if (isset($arr_search['attach_type'])) {
            switch ($arr_search['attach_type']) {
                ...
                case 'none':
                    $_str_sqlWhere .= ' AND `article_attach_id`<1';
                break;
            }
        }
