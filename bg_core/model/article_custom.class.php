...
class MODEL_ARTICLE_CUSTOM {
    ...
    function mdl_create_table($arr_customRows) {
        ...
        foreach ($arr_customRows as $_key=>$_value) {
            $_arr_articleCreat['custom_' . $_value['custom_id']] = 'varchar(300) NOT NULL COMMENT \'自定义字段 ' . $_value['custom_id'] . '\'';
        }

        $_num_db = $this->obj_db->create_table(BG_DB_TABLE . 'article_custom', $_arr_articleCreat, 'article_id', '自定义字段');
        ...
        foreach ($arr_customRows as $_key=>$_value) {
            if (!in_array('custom_' . $_value['custom_id'], $_arr_col)) {
                $_arr_alter['custom_' . $_value['custom_id']] = array('ADD', 'varchar(300) NOT NULL COMMENT \'' . $_value['custom_id'] . ' ' . $_value['custom_name'] . '\'');
            }
