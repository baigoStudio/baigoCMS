...
class MODEL_LINK {
    ...
    function mdl_cache($str_type, $is_reGen = false) {
        ...
        $_str_cacheReturn = $this->obj_file->file_read(BG_PATH_CACHE . 'sys' . DS . 'link_' . $str_type . '.json');
    ...
    function input_submit($arr_input = false) {
        ...
        if ($arr_input && is_array($arr_input) && !fn_isEmpty($arr_input)) {
            $this->linkInput = fn_paramChk($arr_input, $_arr_inputParam);
        } else {
            $this->linkInput = fn_post(false, $_arr_inputParam);
        }

        $this->linkInput['link_id'] = fn_getSafe($this->linkInput['link_id'], 'int', 0);
        ...
        $_arr_linkName = fn_validate($this->linkInput['link_name'], 1, 300);
        ...
        $_arr_linkUrl = fn_validate($this->linkInput['link_url'], 1, 300);
        ...
        $_arr_linkType = fn_validate($this->linkInput['link_type'], 1, 0);
        ...
        $_arr_linkStatus = fn_validate($this->linkInput['link_status'], 1, 0);
        ...
        $_arr_linkCateId = fn_validate($this->linkInput['link_cate_id'], 1, 0);
        ...
        $this->linkInput['link_blank'] = fn_getSafe($this->linkInput['link_blank'], 'int', 0);
    ...
    private function cache_process($str_type) {
        ...
        $_num_size = $this->obj_file->file_put(BG_PATH_CACHE . 'sys' . DS . 'link_' . $str_type . '.json', $_str_outPut);
    }
}
