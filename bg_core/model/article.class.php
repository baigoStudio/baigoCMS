...
class MODEL_ARTICLE {
    ...
    function mdl_create_index() {
        ...
        $_num_db = $this->obj_db->create_index('order_top', BG_DB_TABLE . 'article', $_arr_articleIndex, 'BTREE', $is_exists);
        ...
        $_num_db = $this->obj_db->create_index('order_day', BG_DB_TABLE . 'article', $_arr_articleIndex, 'BTREE', $is_exists);
        ...
        $_num_db = $this->obj_db->create_index('order_week', BG_DB_TABLE . 'article', $_arr_articleIndex, 'BTREE', $is_exists);
        ...
        $_num_db = $this->obj_db->create_index('order_month', BG_DB_TABLE . 'article', $_arr_articleIndex, 'BTREE', $is_exists);
        ...
        $_num_db = $this->obj_db->create_index('order_year', BG_DB_TABLE . 'article', $_arr_articleIndex, 'BTREE', $is_exists);
        ...
        $_num_db = $this->obj_db->create_index('order_all', BG_DB_TABLE . 'article', $_arr_articleIndex, 'BTREE', $is_exists);
     ...
     * @return void
     */
    function mdl_submit($num_adminId, $str_status, $arr_articleSubmit = array()) {
        $_arr_articleData = array(
            'article_status'    => $str_status,
        );

        if (isset($arr_articleSubmit['article_id'])) {
            $_num_articleId = $arr_articleSubmit['article_id'];
        } else if (isset($this->articleInput['article_id'])) {
        ...
        if (isset($arr_articleSubmit['article_title'])) {
            $_arr_articleData['article_title'] = $arr_articleSubmit['article_title'];
        } else if (isset($this->articleInput['article_title'])) {
            $_arr_articleData['article_title'] = $this->articleInput['article_title'];
        }

        if (isset($arr_articleSubmit['article_cate_id'])) {
            $_arr_articleData['article_cate_id'] = $arr_articleSubmit['article_cate_id'];
        } else if (isset($this->articleInput['article_cate_id'])) {
            $_arr_articleData['article_cate_id'] = $this->articleInput['article_cate_id'];
        }

        if (isset($arr_articleSubmit['article_box'])) {
            $_arr_articleData['article_box'] = $arr_articleSubmit['article_box'];
        } else if (isset($this->articleInput['article_box'])) {
            $_arr_articleData['article_box'] = $this->articleInput['article_box'];
        }

        if (isset($arr_articleSubmit['article_time_show'])) {
            $_arr_articleData['article_time_show'] = $arr_articleSubmit['article_time_show'];
        } else if (isset($this->articleInput['article_time_show'])) {
            $_arr_articleData['article_time_show'] = $this->articleInput['article_time_show'];
        }

        if (isset($arr_articleSubmit['article_time_pub'])) {
            $_arr_articleData['article_time_pub'] = $arr_articleSubmit['article_time_pub'];
        } else if (isset($this->articleInput['article_time_pub'])) {
        ...
        if (isset($arr_articleSubmit['article_attach_id'])) {
            $_arr_articleData['article_attach_id'] = $arr_articleSubmit['article_attach_id'];
        } else if (isset($this->articleInput['article_attach_id'])) {
            $_arr_articleData['article_attach_id'] = $this->articleInput['article_attach_id'];
        }

        //print_r($_arr_articleData);

        if (isset($arr_articleSubmit['article_content'])) {
            $_arr_contentData['article_content'] = $arr_articleSubmit['article_content'];
        } else if (isset($this->articleInput['article_content'])) {
            $_arr_contentData['article_content'] = $this->articleInput['article_content'];
        }

        if (isset($arr_articleSubmit['article_source'])) {
            $_arr_contentData['article_source'] = $arr_articleSubmit['article_source'];
        } else if (isset($this->articleInput['article_source'])) {
            $_arr_contentData['article_source'] = $this->articleInput['article_source'];
        }

        if (isset($arr_articleSubmit['article_source_url'])) {
            $_arr_contentData['article_source_url'] = $arr_articleSubmit['article_source_url'];
        } else if (isset($this->articleInput['article_source_url'])) {
            $_arr_contentData['article_source_url'] = $this->articleInput['article_source_url'];
        }

        if (isset($arr_articleSubmit['article_author'])) {
            $_arr_contentData['article_author'] = $arr_articleSubmit['article_author'];
        } else if (isset($this->articleInput['article_author'])) {
     ...
     * @return void
     */
    function input_submit($num_excerptCount = 100) {
        if (!fn_token('chk')) { //令牌
            return array(
                'rcode' => 'x030206',
            );
        ...
        $_arr_articleTitle = fn_validate(fn_post('article_title'), 1, 300);
        ...
        $_arr_articleLink = fn_validate(fn_post('article_link'), 0, 900, 'str', 'url');
        ...
        $_arr_articleStatus = fn_validate(fn_post('article_status'), 1, 0);
        ...
        $_arr_articleBox = fn_validate(fn_post('article_box'), 1, 0);
        ...
        $this->articleInput['article_time_show'] = fn_strtotime(fn_post('article_time_show'));

        $this->articleInput['article_is_time_pub'] = fn_getSafe(fn_post('article_is_time_pub'), 'int', 0);

        if ($this->articleInput['article_is_time_pub'] > 0) {
            $_arr_articleTimePub = fn_validate(fn_post('article_time_pub'), 1, 0, 'str', 'datetime');
        ...
        $this->articleInput['article_is_time_hide'] = fn_getSafe(fn_post('article_is_time_hide'), 'int', 0);

        if ($this->articleInput['article_is_time_hide'] > 0) {
            $_arr_articleTimeHide = fn_validate(fn_post('article_time_hide'), 1, 0, 'str', 'datetime');
        ...
        $_arr_articleCateId = fn_validate(fn_post('article_cate_id'), 1, 0);
        ...
        $this->articleInput['article_content']   = fn_htmlcode(fn_safe(fn_post('article_content')), 'decode');

        $_arr_attachIds = fn_qlistAttach($this->articleInput['article_content']);

        $this->articleInput['article_attach_id'] = $_arr_attachIds[0];

        $this->articleInput['article_source']       = fn_getSafe(fn_post('article_source'), 'txt', '');
        $this->articleInput['article_source_url']   = fn_getSafe(fn_post('article_source_url'), 'txt', '');
        $this->articleInput['article_author']       = fn_getSafe(fn_post('article_author'), 'txt', '');
        ...
        switch ($_str_excerptType) {
            ...
            case 'manual':
                $_arr_articleExcerpt = fn_validate(fn_post('article_excerpt'), 0, 900);
