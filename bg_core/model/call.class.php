...
class MODEL_CALL {
    ...
    function mdl_alter_table() {
        ...
        if (in_array("call_upfile", $_arr_col)) {
            $_arr_alter["call_upfile"] = array("CHANGE", "enum('" . $_str_attachs . "') NOT NULL COMMENT '含有附件'", "call_attach");
        }

        if (in_array("call_attach", $_arr_col)) {
            $_arr_alter["call_attach"] = array("CHANGE", "enum('" . $_str_attachs . "') NOT NULL COMMENT '含有附件'", "call_attach");
        }
        ...
        if (in_array("call_type", $_arr_col)) {
            $_arr_alter["call_type"] = array("CHANGE", "enum('" . $_str_types . "') NOT NULL COMMENT '调用类型'", "call_type");
        }
        ...
        if (in_array("call_file", $_arr_col)) {
            $_arr_alter["call_file"] = array("CHANGE", "enum('" . $_str_files . "') NOT NULL COMMENT '静态页面类型'", "call_file");
        }
    ...
    function input_ids() {
        ...
        $this->callIds = array(
            "rcode"     => $_str_rcode,
            "call_ids"  => array_unique($_arr_callIds),
        );
