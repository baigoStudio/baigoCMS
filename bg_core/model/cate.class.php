...
class MODEL_CATE {
    ...
    function mdl_create_index() {
        ...
        $_num_db = $this->obj_db->create_index('order', BG_DB_TABLE . 'cate', $_arr_cateIndex, 'BTREE', $is_exists);
    ...
    function mdl_cache($num_cateId = 0, $is_reGen = false) {
        ...
        if ($is_reGen || !file_exists(BG_PATH_CACHE . 'sys' . DS . 'cate_trees.json')) {
            ...
            $_num_size      = $this->obj_dir->put_file(BG_PATH_CACHE . 'sys' . DS . 'cate_trees.json', $_str_outPut);
        }
        ...
        if ($is_reGen) {
            ...
            foreach ($arr_cateIds as $_key=>$_value) {
                ...
                if ($_arr_cateRow['rcode'] == 'y250102') {
                    ...
                    $_num_size      = $this->obj_dir->put_file(BG_PATH_CACHE . 'sys' . DS . 'cate_' . $_arr_cateRow['cate_id'] . '.json', $_str_outPut);
                }
            }
        }
        ...
        if ($num_cateId > 0) {
            if (!file_exists(BG_PATH_CACHE . 'sys' . DS . 'cate_' . $num_cateId . '.json')) {
                ...
                if ($_arr_cateRow['rcode'] == 'y250102') {
                    $_str_outPut    = json_encode($_arr_cateRow);
                    $_num_size      = $this->obj_dir->put_file(BG_PATH_CACHE . 'sys' . DS . 'cate_' . $num_cateId . '.json', $_str_outPut);
                }
            }

            $_str_cacheReturn   = file_get_contents(BG_PATH_CACHE . 'sys' . DS . 'cate_' . $num_cateId . '.json');
            ...
            $_str_cacheReturn   = file_get_contents(BG_PATH_CACHE . 'sys' . DS . 'cate_trees.json');
            $_arr_cacheReturn   = json_decode($_str_cacheReturn, true);
        }
    ...
    function mdl_cache_del($arr_cateDels = false) {
        if (is_array($arr_cateDels)) {
            foreach ($arr_cateDels as $_key=>$_value) {
                $this->obj_dir->del_file(BG_PATH_CACHE . 'sys' . DS . 'cate_' . $_value . '.json');
            }
        }
    }
    ...
    function input_submit() {
        ...
        $_arr_cateName = fn_validate(fn_post('cate_name'), 1, 300);
        ...
        $_arr_cateParentId = fn_validate(fn_post('cate_parent_id'), 1, 0);
        ...
        $_arr_cateAlias = fn_validate(fn_post('cate_alias'), 0, 300, 'str', 'alias');
        ...
        $_arr_cateLink = fn_validate(fn_post('cate_link'), 0, 3000);
        ...
        $_arr_cateTpl = fn_validate(fn_post('cate_tpl'), 1, 0);
        ...
        $_arr_cateType = fn_validate(fn_post('cate_type'), 1, 0);
        ...
        $_arr_cateStatus = fn_validate(fn_post('cate_status'), 1, 0);
        ...
        $_arr_cateDomain = fn_validate(fn_post('cate_domain'), 0, 3000);
        ...
        $this->cateInput['cate_content']     = fn_htmlcode(fn_safe(fn_post('cate_content')), 'decode');
